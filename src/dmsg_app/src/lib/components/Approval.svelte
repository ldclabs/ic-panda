<script lang="ts">
  ...
  import CheckoutApproval from './CheckoutApproval.svelte'
  import DocumentApproval from './DocumentApproval.svelte'
  import AuthenticationApproval from './AuthenticationApproval.svelte'
  let requested = $state<'authentication' | 'document' | 'action' | 'checkout' | null>(null)
  let kind = $state<'authentication' | 'document' | 'action' | 'checkout' | null>(null)
  onMount(() => {
    ...
    void listRequests()
      .then((rows) => {
        ...
        if (
          !record ||
          !['authentication', 'document', 'action', 'checkout'].includes(record.kind)
        )
...
{#if kind === 'authentication'}<AuthenticationApproval
  />{:else if kind === 'checkout'}<CheckoutApproval
  />{:else if kind === 'document' || kind === 'action'}<DocumentApproval
  />{:else if session.error}<p role="alert">
    {session.error}
  </p>{/if}
