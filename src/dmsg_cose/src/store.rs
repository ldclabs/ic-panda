...
thread_local! {
    ...
    static EXECUTIONS: RefCell<StableBTreeMap<Vec<u8>, Stored<model::Execution>, Memory>> = RefCell::new(StableBTreeMap::init(memory(2)));
    pub(crate) static HOMES: RefCell<StableBTreeMap<Vec<u8>, Stored<model::Home>, Memory>> = RefCell::new(StableBTreeMap::init(memory(1)));
}
