<script lang="ts">
  ...
  let currentSupply: bigint | null = $state(null)
...
<SectionShell id="panda" index="02" label={$t('PANDA · Governance Token')}>
  ...
  <Reveal>
    <div class="border-ink/15 grid grid-cols-1 border-t sm:grid-cols-2">
      <div class="border-ink/10 border-b py-6 sm:border-b-0 sm:pr-6">
        <p class="eyebrow">{$t('Genesis Supply')}</p>
        <p class="mt-3 font-mono text-2xl font-medium tabular-nums md:text-3xl">
          {BigInt(GENESIS_SUPPLY.replaceAll(',', '')).toLocaleString($locale)}
        </p>
      ...
      <div class="sm:border-ink/10 py-6 sm:border-l sm:pl-6">
        <p class="eyebrow">
          ...
          <span
            ...
            class:bg-panda={currentSupply !== null}
        ...
        {#if currentSupply !== null}
          ...
          >
            {currentSupply.toLocaleString($locale)}
          </p>
          <p class="text-ink-70 mt-1 font-mono text-xs tracking-[0.14em]">
            {$t('PANDA · live from the ledger')}
          </p>
        {:else}
          <p class="mt-3">
            <LinkOut
              href={LINKS.ledgerCanister}
              label={$t('Verify on-chain')}
              class="!text-base"
            />
          </p>
          <p class="text-ink-70 mt-2 font-mono text-xs tracking-[0.14em]">
            {$t('PANDA · reading the ledger')}
          </p>
        {/if}
      </div>
    </div>
      ...
      {#each TOKEN_FACTS as fact, i (fact.label)}
        <div
          class="border-ink/10 border-b py-6 sm:border-b-0 sm:px-6 {i > 0
            ...
            : 'sm:pl-0'}"
        >
          <p class="eyebrow">{$t(fact.label)}</p>
          <p
  ...
  <Reveal>
    ...
    >
      <div class="min-w-0">
        <p class="eyebrow">{$t('Canonical ICP Ledger')}</p>
      ...
      <LinkOut
        ...
        label={$t('View Official SNS Data')}
        variant="outline"
        class="shrink-0"
      />
    </div>
  ...
  <div class="border-ink/15 mt-16 border-t pt-14 md:mt-24 md:pt-20">
    <div class="grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-12">
      <div class="lg:col-span-5">
        <Reveal>
          <p class="eyebrow">{$t('Genesis Allocation')}</p>
          <h3 class="display mt-5 text-[clamp(2rem,4.2vw,3rem)]">
            {$t('Built for community ownership.')}
          </h3>
          <p
      ...
      <div class="lg:col-span-7">
        <Reveal delay={80}>
          ...
          <div class="border-ink/20 mt-8 border-l-2 pl-5">
            <p class="eyebrow">{$t('Historical allocation')}</p>
          ...
          <div class="mt-8">
            <LinkOut
              ...
              label={$t('Explore On-chain Tokenomics')}
            />
          </div>
        </Reveal>
      </div>
  ...
  <div class="border-ink/15 mt-16 border-t pt-14 md:mt-24 md:pt-20">
    <Reveal>
      <p class="eyebrow">{$t('What PANDA Does')}</p>
    </Reveal>
    <Reveal>
      <div
      ...
      >
        {#each PANDA_ROLES as role (role.name)}
          <div class="h-full bg-white p-6 md:p-8">
            ...
            <h4 class="display-sm mt-6 text-3xl">{$t(role.name)}</h4>
  ...
  <Reveal>
    ...
    >
      <p class="eyebrow">{$t('Trade PANDA')}</p>
      <div class="flex flex-wrap items-center gap-x-6 gap-y-3">
        <LinkOut href={LINKS.icpswap} label="ICPSwap" />
        <LinkOut href={LINKS.bridge} label={$t('Bridge to BNB')} />
