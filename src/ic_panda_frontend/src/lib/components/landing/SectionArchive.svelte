...
<SectionShell id="archive" index="06" label={$t('Our History')}>
    ...
    <div class="lg:col-span-6">
      <Reveal>
        <h2 class="display text-[clamp(2.25rem,5vw,3.5rem)]">
          {$t('Engineering that')}<br />{$t('carries forward.')}
        </h2>
      </Reveal>
    </div>
  ...
  <Reveal>
    ...
    >
      ...
      >
        <div>
          <h3 class="display-sm text-3xl">{$t('Early dMsg')}</h3>
          <p class="text-ink-70 mt-2 font-mono text-xs"
            >{$t('Encrypted messaging')}</p
          >
        </div>
        <span
      ...
      <div class="p-6 md:col-span-8 md:p-8">
        ...
        <div class="mt-8 flex flex-wrap items-center gap-x-8 gap-y-4">
          <LinkOut
            href={LINKS.dmsgGithub}
            label={$t('Explore the Earlier Source')}
          />
          <a class="link-mark" href="#dmsg">{$t('Explore dMsg')}</a>
        </div>
      </div>
    </article>
  </Reveal>
