...
// These prefixes are produced by the legacy decoder, not by user messages.
// Keep the diagnostic detail verbatim while localizing its explanatory label.
export const errorText = derived(t, ($t) => (error: string): string => {
  const prefixes = [
    'Failed to decrypt message: ',
    'Failed to decode message: ',
    'Failed to decode payload: '
  ]
  const prefix = prefixes.find((value) => error.startsWith(value))
  return prefix
    ? $t(prefix + '{error}', { error: error.slice(prefix.length) })
    : $t(error)
})
