<script lang="ts">
  import { t, locale, localeDir } from '$lib/i18n'
...
>
  ...
  <Popover.Portal>
    ...
    >
      <p class="px-3 py-2 font-semibold">{$t('Legacy account · Read-only')}</p>
      <a href="/" data-sveltekit-reload>{$t('Back to dMsg')}</a>
      <a href="/legacy">{$t('Archive information')}</a>
      {#if $authStore.identity.getPrincipal().isAnonymous()}
        <a href="/legacy">{$t('Sign in to read your history')}</a>
      {:else}
        {#if authStore.srcIdentity}
          <button
            ...
            >{$t('Original identity ·')}
            {shortId(authStore.srcIdentity.getPrincipal().toText())}</button
          >
        {/if}
        ...
        {#if loading}<p class="px-3 py-2" role="status"
            >{$t('Loading username accounts…')}</p
          >{/if}
        {#if switching}<p class="px-3 py-2" role="status"
            >{$t('Switching identity…')}</p
          >{/if}
        {#if error}<p class="px-3 py-2 text-red-800" role="alert">{$t(error)}</p
          ><button onclick={loadAccounts}>{$t('Retry account list')}</button
          >{/if}
        <button onclick={() => authStore.logout('/legacy')}
          >{$t('Sign out')}</button
        >
      {/if}
    </Popover.Content>
