...
thread_local! {
    ...
    static NAMES:RefCell<Table>=RefCell::new(Table::new(1));
    static LEGACY:RefCell<Table>=RefCell::new(Table::new(2));
    static OPS:RefCell<Table>=RefCell::new(Table::new(3));
    static LOCKS:RefCell<Table>=RefCell::new(Table::new(4));
    static EVENTS:RefCell<Table>=RefCell::new(Table::new(5));
    static SUBJECT_OPS:RefCell<Table>=RefCell::new(Table::new(6));
    static TRANSFERS:RefCell<Table>=RefCell::new(Table::new(7));
...
fn op_key(subject: Hash, op: Hash) -> Hash {
    digest("dmsg/handle-operation/v1", &(subject, op))
}
fn record(name: &str) -> Option<HandleRecord> {
...
fn import_legacy_handles(
    ...
    for e in &entries {
        ...
        if let Some(old) = LEGACY.with_borrow(|t| t.get::<LegacyReservation>(e.handle.as_bytes())) {
...
fn commit_name(name: &str, from: Option<Hash>, to: Hash, version: u64) -> HandleRecord {
    let mut c = cfg();
...
async fn claim_legacy_handle(intent: HandleIntent, snapshot_id: Hash) -> Result<HandleRecord> {
    ...
    let legacy = LEGACY
        .with_borrow(|t| t.get::<LegacyReservation>(intent.handle.as_bytes()))
    ...
    let owner_is_name = legacy.legacy_name_principal == Some(legacy.legacy_owner);
        ...
        intent.target_subject.is_none()
            && intent.expected_version == 0
            && intent.terms_digest
                == digest(
                    "dmsg/legacy-claim/v1",
                    &(snapshot_id, &legacy, intent.subject),
                ),
        Error::IntegrityFailed,
    )?;
    if let Some(r) = record(&intent.handle) {
        ensure(
            r.owner_subject == intent.subject && r.version == 1,
    ...
    Ok(commit_name(&intent.handle, None, intent.subject, 1))
}
...
async fn reserve_handle(registration: Registration) -> Result<HandleOperation> {
    ...
    let key = op_key(i.subject, i.op_id);
...
fn finish_paid(key: &Hash, block: u64) -> Result<HandleOperation> {
    ...
    let i = &o.registration.intent;
        ...
        LOCKS.with_borrow(|t| t.get::<Hash>(i.handle.as_bytes())) == Some(*key)
            && record(&i.handle).is_none(),
        Error::VersionConflict,
    )?;
    ...
    save_op(key, &o);
    commit_name(&i.handle, None, i.subject, 1);
...
#[ic_cdk::update]
async fn commit_handle(subject: Hash, op_id: Hash) -> Result<HandleOperation> {
    let key = op_key(subject, op_id);
    let mut o = op(&key)?;
    ...
    match response {
        Ok(Ok(block)) => finish_paid(&key, ledger::block_index(block)?),
        Ok(Err(TransferFromError::Duplicate { duplicate_of })) => {
            finish_paid(&key, ledger::block_index(duplicate_of)?)
        }
        Ok(Err(_)) => {
            o = op(&key)?;
            if was_unknown {
...
async fn reconcile_handle_charge(
    ...
    let tx = ledger::read_transfer(cfg().init.ledger, block).await?;
    ensure(
...
#[ic_cdk::update]
fn expire_handle_reservation(subject: Hash, op_id: Hash) -> Result<()> {
    let key = op_key(subject, op_id);
    let mut o = op(&key)?;
    ensure(
...
async fn transfer_handle(from: HandleIntent, accept: HandleIntent) -> Result<HandleRecord> {
    ...
    check_intent(&accept, HandleAction::AcceptTransfer)?;
        ...
        from.subject != accept.subject
            && from.target_subject == Some(accept.subject)
            && accept.target_subject == Some(from.subject)
...
#[ic_cdk::query]
fn get_handle_operation(subject: Hash, op_id: Hash) -> Result<HandleOperation> {
    op(&op_key(subject, op_id))
}
#[ic_cdk::query]
fn get_legacy_reservation(handle: String) -> Result<Option<LegacyReservation>> {
    let name = normalize_handle(&handle)?;
    Ok(LEGACY.with_borrow(|t| t.get(name.as_bytes())))
}
#[ic_cdk::query]
...
fn list_legacy_reservations(after: Option<String>) -> Result<Vec<LegacyReservation>> {
    ...
    Ok(LEGACY
        .with_borrow(|t| t.page::<LegacyReservation>(key, 64))
        .into_iter()
        .map(|(_, v)| v)
        .collect())
}
...
fn get_handle_event(sequence: u64) -> Option<HandleEvent> {
    EVENTS.with_borrow(|t| t.get(&sequence.to_be_bytes()))
}
