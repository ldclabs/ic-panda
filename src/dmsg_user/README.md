...
签名在商业缓存有效时只做一次完整授权；确需跨服务刷新时，仍在调用前预检查，返回后重读时间、账户与执行记录。显式 `refresh_execution_entitlement` 始终获取当前商业权益，并保留当月已用和预留额度。每次认证叶写入都重新发布根，消息结束时以最后一次为准；升级重建整批设置叶后只发布一次。`reconcile_execution` 对 COSE 查询的传输失败直接返回错误，不改写执行记录。
