...
schema 5 的私有 `stable_codec.rs` 递归使用 CBOR 整数 map key，并省略空窗口、空授权和 `None` 字段；公共账户、批准与认证编码不变。稳定 memory ID 只属于本实现。满设备、操作、名称授权和执行索引的旧 schema 4 测试样本编码为 17,642 字节；schema 5 另含创建时间、商业授权和安全预算；其中新增保留索引以少量常驻字节换取不扫描历史载荷，样本不包括独立执行表、认证树和稳定表节点开销。
