...
export type CborValue = null | boolean | bigint | string | Uint8Array | CborValue[] | { [key: string]: CborValue }
...
function head(major: number, value: bigint): Uint8Array {
  ...
  const size = value <= 255n ? 1 : value <= 65535n ? 2 : value <= 4294967295n ? 4 : 8
  const result = new Uint8Array(size + 1)
  result[0] = (major << 5) | ({ 1: 24, 2: 25, 4: 26, 8: 27 }[size]!)
  for (let i = size; i > 0; i--) { result[i] = Number(value & 255n); value >>= 8n }
  return result
}
...
export function canonical(value: unknown, depth = 0): Uint8Array {
  ...
  if (typeof value === 'bigint') {
    ...
    return concat(Uint8Array.of(0xc2), head(2, BigInt(bytes.length)), Uint8Array.from(bytes))
  }
  if (typeof value === 'string') {
    if (new TextDecoder().decode(utf8.encode(value)) !== value) throw new Error('Invalid Unicode')
  ...
  if (Array.isArray(value)) return concat(head(4, BigInt(value.length)), ...value.map(v => canonical(v, depth + 1)))
  if (typeof value === 'object' && Object.getPrototypeOf(value) === Object.prototype) {
    const pairs = Object.entries(value).map(([k, v]) => [canonical(k, depth + 1), canonical(v, depth + 1)])
...
export async function digest(domain: string, value: unknown): Promise<Uint8Array> {
...
export function decodeCanonical(bytes: Uint8Array): CborValue {
  ...
  function read(depth: number): CborValue {
    ...
    if (ai >= 24) {
      ...
      n = 0n; for (const byte of take(count)) n = (n << 8n) | BigInt(byte)
    }
    if (major === 0) return n
    if (major === 6) {
      if (n !== 2n) throw new Error('Unsupported CBOR tag')
      const magnitude = read(depth + 1)
      if (!(magnitude instanceof Uint8Array) || magnitude.length < 9 || magnitude.length > 16 || magnitude[0] === 0) throw new Error('Invalid u128')
      let value = 0n; for (const byte of magnitude) value = (value << 8n) | BigInt(byte)
      return value
    }
    if (n > BigInt(bytes.length)) throw new Error('CBOR length limit')
    const count = Number(n)
    if (major === 2) return take(count)
    if (major === 3) return new TextDecoder('utf-8', { fatal: true }).decode(take(count))
    if (major === 4) return Array.from({ length: count }, () => read(depth + 1))
    if (major === 5) {
      const value: Record<string, CborValue> = {}
      for (let i = 0; i < count; i++) {
        const key = read(depth + 1)
        if (typeof key !== 'string' || Object.hasOwn(value, key)) throw new Error('Invalid map key')
        Object.defineProperty(value, key, { value: read(depth + 1), enumerable: true, configurable: true, writable: true })
      }
      return value
    }
  ...
  const value = read(0)
  if (offset !== bytes.length || !equalBytes(canonical(value), bytes)) throw new Error('Noncanonical CBOR')
  return value
}
