...
SUMMARY="To ensure ICPanda's development and critical operations remain uninterrupted, we propose to temporarily withdraw the liquidity from ICPSwap pool $POOL by closing position $POSITION_ID, with the proceeds held by the team account $TO_PRINCIPAL and used only for essential protocol continuity and operations for no longer than 3 months; we will publish periodic, easy-to-audit updates (balances and on-chain references), and once conditions normalize we will compensate the withdrawn amount back by creating a new position and re-adding liquidity to pool $POOL. Previous proposal: https://dashboard.internetcomputer.org/sns/d7wvo-iiaaa-aaaaq-aacsq-cai/proposal/446"
