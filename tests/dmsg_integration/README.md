...
`control_plane/membership.rs` covers approval-before-allocation, admission races, expired subsidy recovery across subjects/upgrades, known stake loss during cooling, admission pause inside the SNS callback, terminal close replay, policy-time conflicts, and Apply/Close ACK loss followed by upgrade/reconciliation. The SNS test double has a fault-only permissive adapter and call counters; it is never a production admission path. The ignored `membership_cycles_profile` compares admission, activation, refresh and upgrade with 0/8/32 cancelled historical claims; see the membership README for results and scope.
