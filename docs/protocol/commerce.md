...
Membership verifies initial account approval before allocating a Claim, application slot or subsidy reservation; authorization failures leave no claim record. Attempts have separate limits of 10 per actor and 200 globally per minute, with independent 200/minute budgets for qualification and active refresh. Pending known ineligibility releases the application, and restored stake must cool again under a new application. Admission pause is rechecked after callbacks before preparing Apply. `sweep_expired_claims()` and authorized admission each process at most 32 due records from a stable deadline index; unknown Apply/Close results never expire by time. Released claims stay terminal on repeated close or reconciliation. First delivery sends the immutable decision directly; recovery queries its original receipt. Membership configuration is development schema 2, Claim encoding remains schema 2, and hot counters persist through `pre_upgrade`. The hook must run for same-schema upgrades. Initial product validation, effective-policy indexing and profile results are documented in [the membership README](../../src/membership/README.md).
