...
Stable layout versions are maintained by each canister's `store.rs`, tested using new instances, and do not read development state from previous schemas. `dmsg_handle` schema 5 uses `StableLog` for events, fixed-byte name locks, account-to-operation mappings, and a bounded expiry index. It adds individual certified legacy records, same-operation transient retries, and fee maintenance for new registrations while retaining small allocation buckets; cycles comparisons and capacity limits are documented in its [README](../src/dmsg_handle/README.md). Integer keys and representative sample bytes are pinned by round-trip, size threshold, `StableBTreeMap` allocation, and SHA-256 golden tests. Execution recovery after code upgrades on the same schema is covered by PocketIC. Certified trees continue using public protocol encodings and are reconstructed from stable records, so compact stable representations do not affect certified responses.
...
The shared runtime stores compact representations directly, avoiding the extra domain-object clone before map writes. Commercial reservations and fee policies also use integer keys. Development layouts are user schema 6 and payment schema 5; other layout versions are unchanged. Certified batches enforce the 256 KiB limit on the complete successful Candid response, including the certificate and envelope.
